from Tools import *
from TrajectoryExp import TrajectoryExp
...
class ExperimentExp:
    ...
    def __init__(self, traces):
        ...
        for trace in range(1, traces + 1):
            ...
            self.__traces.append(TrajectoryExp('data_test.xls', trace))
        ...
        self.__data_total.to_csv("data_exp/total.txt", header=True, index=False, sep=' ', mode='w')
        total_trace = TrajectoryExp("total.txt", case='_total', p=self.p_mean, k=self.k_mean)

        my_file = Path(os.path.join(os.path.dirname(__file__), 'Trajectory_tables_exp_txt/', total_trace.path[-8:-4]))
        if my_file.is_file():
            total_trace.histo_data = pd.read_csv(os.path.join(os.path.dirname(__file__), 'Trajectory_tables_exp_txt/',
                                                              'total.txt'), sep=" ", header=0)
        else:
        ...
        results_dir = os.path.join(script_dir, 'Work_histograms_exp/')
        if not os.path.isdir(results_dir):
            os.makedirs(results_dir)
        results_dir_rup = os.path.join(script_dir, 'Force_histograms_exp/')
        if not os.path.isdir(results_dir_rup):
            os.makedirs(results_dir_rup)
        text_file = open("Work_histograms_exp/works" + str(len(self.__traces)) + ".txt", "w")
        text_file.write(str(self.states))
        text_file.close()
        text_file = open("Force_histograms_exp/forces" + str(len(self.__traces)) + ".txt", "w")
    ...
    def plot_work_histograms(self):
        ...
        script_dir = os.path.dirname(__file__)
        results_dir_work = os.path.join(script_dir, 'Work_histograms_exp/')

        for state in self.states:
            self.plot_histogram(values=self.states[state], label='Work', bins=70, range=[min(self.states[state])-20,
                                                                                         max(self.states[state])+20])
            plt.tight_layout()
        ...
        for state in self.states_rup:
            ...
            # Plot the histogram.
            # Plot the PDF.
            f_space = np.linspace(10, 30, 1000)
            p = norm.pdf(f_space, mu, std)
            plt.plot(f_space, p, ls='--', linewidth=0.5, color=get_color(i))
            plt.hist(self.states_rup[state], bins=50, density=True, alpha=0.3, color=get_color(i), range=[10, 30])
